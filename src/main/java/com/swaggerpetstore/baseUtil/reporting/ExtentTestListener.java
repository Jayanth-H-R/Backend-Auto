...
public class ExtentTestListener implements ITestListener, ISuiteListener, IConfigurationListener {
	...
	private static final Map<ITestNGMethod, AtomicInteger> testMethodInvocationCount = new ConcurrentHashMap<>();
	...
	private static final AtomicInteger testCaseCount = new AtomicInteger(0); // Total count of all test cases
	...
	public synchronized void onFinish(ISuite suite) {
		...
		log.info("Total Test Cases: " + testCaseCount.get() + ", Executed: " + totalExecuted + ", Passed: "
				+ passCount.get() + ", Skipped: " + skippedCount.get() + ", Failed: " + failTestCaseCount.get());
	...
	public synchronized void onTestStart(ITestResult result) {
		...
		ExtentTest parent = parentTestMap.computeIfAbsent(parentName, name -> {
	...
	public synchronized void onTestSuccess(ITestResult result) {
		...
		childTest.get().pass(
				MarkupHelper.createLabel("Test Passed - " + methodName + " (" + dataSetName + ")", ExtentColor.GREEN));
	}
	...
	public synchronized void onTestFailure(ITestResult result) {
		...
		String dataSetName = getDataSetName(result);
		childTest.get().fail(
				MarkupHelper.createLabel("Test Failed - " + methodName + " (" + dataSetName + ")", ExtentColor.RED));
	...
	private String getDataSetName(ITestResult result) {
		...
		AtomicInteger counter = testMethodInvocationCount.computeIfAbsent(method, m -> new AtomicInteger(0));
