...
public class AddPetValidator extends BaseValidator {
	/**
	 * This class is responsible for validating the response of the Add New Pet API
	 * call It extends the BaseValidator class to utilize common validation methods.
	 * The validateAddPet method checks if the API call was successful and maps the
	...
	public void validateGetPetById(ApiCallData apiCallData, ApiCallDataMapper apiCallDataMapper) {
		...
		verifySuccessStatusCode();
		AddNewPetResponse response = apiCallDataMapper.getResponseDto(apiCallData, AddNewPetResponse.class);
		// GetPetByIdRequest request = apiCallDataMapper.getRequestDto(apiCallData,
	...
	public void validateUpdatePet(ApiCallData apiCallData, ApiCallDataMapper apiCallDataMapper) {
		...
		Assert.assertEquals(response.getCategory().getId(), request.getCategory().getId(),
				"Updated Pet Category ID mismatch");
	}
